...
In this example, myNode can be any node in the SharedTree.
It will be optionally passed into the callback function.
...
You can also pass a `TreeView` to `runTransaction()`:
...
The alpha APIs introduce [TreeBranchAlpha.runTransaction](../../api/fluid-framework/treebranchalpha-interface#runtransaction-methodsignature) as a replacement for `Tree.runTransaction`.
...
Transactions are started via [runTransaction](../../api/fluid-framework/treebranchalpha-interface#runtransaction-methodsignature) on any [TreeBranchAlpha](../../api/fluid-framework/treebranchalpha-interface)
...
The callback function passed to runTransaction implements the transaction logic.
It can optionally return a status object indicating success or failure, and include a user-defined value.
...
Constraints let you opt in to stricter validation when your application needs it.
A violated constraint causes the **entire transaction** to be dropped.
Constraints can be applied as preconditions to a transaction (or as preconditions on revert, to be applied if the transaction is reverted) thereby ensuring that the transaction's changes will only be applied if some specific conditions are met.
