...
A `ServiceClient` is obtained from a per-service factory and exposes `createContainer` and `loadContainer`. Container contents are described by a `DataStoreKind` rather than a container schema; `treeDataStoreKind` (from `fluid-framework` / `@fluidframework/tree`) builds one from a `SharedTree` view configuration, and `dataStoreKind` builds one from any root shared object.
...
- `createEphemeralServiceClient` (`@fluidframework/local-driver`, in-memory, for tests)
...
const appKind = treeDataStoreKind({
	type: "my-app",
