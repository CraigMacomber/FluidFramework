...
/**
 * A bunch of messages sharing the same envelop properties.
 * @remarks
 * Processed by {@link ContainerRuntime.validateAndProcessRuntimeMessages}.
 */
interface MessageBunch {
	/** The core message with common properties for all the messages in the bunch. */
	message: Omit<InboundSequencedContainerRuntimeMessage, "contents">;
	/** The contents, local metadata and clientSequenceNumbers of the messages. */
	messagesContent: IRuntimeMessagesContent[];
	/** true if the messages were originally generated by the client receiving it. */
	local: boolean;
	/** true if the message is a replayed saved op. */
	savedOp?: boolean;
}

/**
 * A batch of sequential messages to process together as an optimization.
 */
type MessageBunchBatch = MessageBunch[];
		...
		IProvideLayerCompatDetails
{
	...
	private processInboundMessages(
		...
		try {
			...
			const bunches: MessageBunchBatch = [];
			...
			// Push the last bunch and process all bunches together.
			assert(previousMessage !== undefined, "expected at least one bunch");
			bunches.push({
				message: previousMessage,
				messagesContent: bunchedMessagesContent,
				local,
			...
			this.validateAndProcessRuntimeMessages(bunches);
	 ...
	 * Assuming the messages in the given bunches are also a TypedContainerRuntimeMessage, checks their type and
	 * dispatches the messages to the appropriate handler in the runtime.
	 * Consecutive bunches destined for the channel collection are passed together in a single call to maximize
	 * batching efficiency.
	 * Throws a DataProcessingError if a message looks like but doesn't conform to a known TypedContainerRuntimeMessage type.
	 * @param bunches - The batch of message bunches to process.
	 */
	private validateAndProcessRuntimeMessages(bunches: MessageBunchBatch): void {
		const channelCollectionBunches: IRuntimeMessageCollection[] = [];

		const flushChannelCollectionBunches = (): void => {
			if (channelCollectionBunches.length > 0) {
				this.channelCollection.processMessages(channelCollectionBunches);
				channelCollectionBunches.length = 0;
			}
		};

		for (const { message, messagesContent, local, savedOp } of bunches) {
			switch (message.type) {
				case ContainerMessageType.FluidDataStoreOp:
				case ContainerMessageType.Attach:
				case ContainerMessageType.Alias: {
					// Remove the metadata from the messages before sending them to the channel collection. The metadata
					// is added by the container runtime and is not part of the messages that the channel collection and
					// layers below it expect.
					channelCollectionBunches.push({ envelope: message, messagesContent, local });
					break;
				}
				default: {
					flushChannelCollectionBunches();
