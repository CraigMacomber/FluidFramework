...
import { Type, type TSchema, type TUnsafe } from "@sinclair/typebox";
 ...
 * @deprecated VersionedEncoded should be used.
 */
type VersionedJson = JsonCompatibleReadOnlyObject & Versioned;

/**
 * A type constraint for encoded codec content.
 * @remarks
 * Currently we have some cases which allow handles, and some which do not, and this tolerates both explicitly.
 */
type VersionedEncoded = JsonCompatibleReadOnlyObject<IFluidHandle> & Readonly<Versioned>;
const VersionedEncoded = Versioned as TUnsafe<VersionedEncoded>;
...
function makeVersionedCodec<
	TDecoded,
	TEncoded extends Versioned = VersionedEncoded,
	TValidate = TEncoded,
	TContext = void,
>(
...
} & IJsonCodec<TDecoded, VersionedEncoded, VersionedEncoded, TContext>;
...
> {
	/**
	 * When undefined, codec is considered unstable,
	 * and will never be selected as a write version unless via override.
	 */
	readonly minVersionForCollab: MinimumVersionForCollab | "unstable";
...
> {
	...
	private constructor(
		...
		for (const codec of inputRegistry) {
			...
			if (codec.minVersionForCollab !== "unstable") {
				debugAssert(
					() =>
	...
	private buildInternal(
		...
		return {
			encode: (data: TDecoded, context: TContext): VersionedEncoded => {
				return writeVersion.codec.encode(data, context);
			},
			decode: (data: VersionedEncoded, context: TContext): TDecoded => {
...
The client which encoded this data likely specified an "minVersionForCollab" value which corresponds to a version newer than the version of this client ("${pkgVersion}").`,
	 ...
	 */
	public build(options: TBuildOptions): IJsonCodec<TDecoded, TEncoded, TEncoded, TContext> {
		return this.buildInternal(options) as IJsonCodec<TDecoded, TEncoded, TEncoded, TContext>;
	}
	...
	/**
	 * Unsafely cast "this" such that it produces a codec with `TEncodedNew` as the type for encoded data.
	 * @remarks
	 * A workaround for providing a more specific type for the union of all encoded formats for all versions of this codec.
	 * See notes on {@link withSchemaValidation}.
	 *
	 * TODO: this unsafety should be either avoided made more safe
	 */
	public retypeEncodedUnsafe<TEncodedNew>(): ClientVersionDispatchingCodecBuilder<
		Name,
		TDecoded,
		TContext,
		TFormatVersion,
		...
		TEncodedNew
	> {
		return this as unknown as ClientVersionDispatchingCodecBuilder<
			Name,
			TDecoded,
			TContext,
			TFormatVersion,
	...
	/**
	 * A TypeBox schema for use in parent codecs which contain output from this codec.
	 * @remarks
	 * This does no validation beyond {@link Versioned}, and instead simply brands the type as the encoded type for this codec.
	 * @privateRemarks
	 * TODO: See notes on withSchemaValidation for how we might want to add branding or other safety improvements here.
	 * TODO: We could have this do some validation (like enforce that data is versioned) to make this a bit more safe.
	 */
	public readonly unenforcedSchemaForCodecNesting =
		VersionedEncoded as unknown as TUnsafe<TEncoded>;
	...
	>(name: Name, inputRegistry: readonly Entry[]) {
		...
		const builder = new ClientVersionDispatchingCodecBuilder<
			...
			TFormatVersion2,
			TBuildOptions2,
			// Default TEncoded value. See notes on withSchemaValidation for how we may want to make this more safe in the future.
			// See retypeEncodedUnsafe for adjusting this
			VersionedEncoded
		>(name, input);
		return builder;
	}
...
function getWriteVersion<T extends CodecVersionBase>(
	...
	if (options.writeVersionOverrides?.has(name) === true) {
		...
		if (selected === undefined) {
			...
			// Unstable version selected
			if (selectedMinVersionForCollab === undefined) {
				throw new UsageError(
					`Codec "${name}" does not support requested format version ${selectedFormatVersion} because it is unstable. Use "allowPossiblyIncompatibleWriteVersionOverrides" to suppress this error if appropriate.`,
				);
...
function getWriteVersionNoOverrides<T extends CodecVersionBase>(
	...
	for (const version of versions) {
		if (version.minVersionForCollab !== "unstable") {
